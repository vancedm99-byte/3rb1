...
const logger = new Logger('MiraVdExtractor');

/**
 * Extractor for miravd.com / miravd embeds
 * Commonly used as Server 1 / mirror host on 3isk
 */
export async function extractMiraVd(url: string, referer?: string): Promise<ResolvedStream[]> {
  try {
    ...
    logger.debug(`MiraVd extraction failed for ${url}: ${(err as Error).message}`);
    return [];
  }
}
