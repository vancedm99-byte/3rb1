...
const logger = new Logger('VidobaExtractor');

/**
 * Extractor for vidoba.org embeds
 * Commonly used as Server 3 / mirror host on 3isk
 */
export async function extractVidoba(url: string, referer?: string): Promise<ResolvedStream[]> {
  try {
    ...
    logger.debug(`Vidoba extraction failed for ${url}: ${(err as Error).message}`);
    return [];
  }
}
